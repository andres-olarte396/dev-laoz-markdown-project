        ...
        if (state.topicId) {
          setTimeout(() => {
            const topicLink = document.querySelector(
              `.topic-item[onclick*="${state.topicId}"]`
            );
            if (topicLink) {
              topicLink.click();
            } else {
  ...
  try {
    ...
    if (data.success) {
      ...
      document.getElementById(
        "backendStatus"
      ).textContent = `✅ Conectado (v${data.version})`;
    }
  } catch (error) {
    document.getElementById("apiStatus").textContent = "🔴";
    ...
    if (data.success && data.data) {
      ...
      if (courses.length === 0) {
        ...
        courseList.innerHTML = courses
          .map(
            (course) => `
                    <div class="course-item" onclick="loadCourseDetail('${
                      course.id
                    }')">
...
async function loadCourseDetail(courseId) {
  ...
  const activeItem = document.querySelector(
    `.course-item[onclick="loadCourseDetail('${courseId}')"]`
  );
  ...
  modulesList.innerHTML =
    '<div class="loading">Cargando estructura del curso...</div>';
    ...
    if (data.success && data.data) {
      ...
      if (course.modules) {
        course.modules.forEach((m) =>
          console.log(
            `Module ${m.id} has ${m.topics ? m.topics.length : 0} topics`
          )
        );
      }
      ...
      document.getElementById("courseDuration").textContent =
        course.duration_hours
          ...
          : "No especificado";
      document.getElementById("courseModules").textContent =
            ...
            (module, index) => `
                    <div class="module-item">
                        <div class="module-header" onclick="toggleModule(${index})">
                            <h4>📘 ${
                              module.title || `Módulo ${module.module_number}`
                            }</h4>
        ...
        <div class="topic-list">
            ${topics
              .map((topic) => {
                ...
                const safeEval = (topic.evaluation_path || "").replace(
                  /\\/g,
                  "/"
                );
                ...
                <div class="topic-item ${isCompleted ? "completed" : ""}" 
                    ...
                    <div class="topic-badges">
                        ${
                          topic.audio_path
                            ? '<span class="topic-badge">🎧 Audio</span>'
                            : ""
                        }
                        ${
                          topic.evaluation_path
                            ? '<span class="topic-badge quiz">📝 Quiz</span>'
                            : ""
                        }
                    </div>
                </div>
...
async function loadTopicContent(topicId, audioPath, evalPath) {
  console.log("DEBUG: loadTopicContent called", {
    topicId,
    audioPath,
    evalPath,
  });
  ...
  if (audioPath && audioPath !== "null" && audioPath !== "undefined") {
    ...
    btnAudio.onclick = () =>
      setupAudioPlayer(topicId, currentTopicTitle || "Audio del tema");
  } else {
  ...
  try {
    const response = await fetch(
      `${API_BASE}/content/${encodeURIComponent(topicId)}`
    );
    const data = await response.json();
    ...
    if (data.success && data.data) {
      ...
      try {
        ...
        console.warn("Progress logic error", pErr);
      }
...
function loadNextTopic() {
  ...
  if (currentIndex !== -1 && currentIndex < allTopics.length - 1) {
    ...
    loadTopicContent(
      nextTopic.id,
      nextTopic.audio_path,
      nextTopic.evaluation_path
    );
  }
}
...
function loadPrevTopic() {
  ...
  if (currentIndex > 0) {
    ...
    loadTopicContent(
      prevTopic.id,
      prevTopic.audio_path,
      prevTopic.evaluation_path
    );
  }
}
...
async function startQuiz() {
  ...
  try {
    const response = await fetch(
      `${API_BASE}/content/${encodeURIComponent(currentTopic)}/evaluation`
    );
    const data = await response.json();
    ...
    if (currentQuizQuestions.length === 0) {
      alert(
        "No se pudieron detectar preguntas en esta evaluación. Verifica el formato del archivo."
      );
      return;
    }
...
function submitQuiz() {
  ...
  currentQuizQuestions.forEach((q, index) => {
    ...
    let correctAnswerVal = correctOption
      ? correctOption.letter || "marked"
      : "unknown";
    ...
    if (selected) {
      ...
      if (correctOption && selectedVal === correctOption.letter) {
        ...
        correctOption &&
        typeof correctOption.letter === "undefined" &&
